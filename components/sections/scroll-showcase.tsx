...
function StepPanel({ step, opacityFrom, opacityPeakStart, opacityPeakEnd, opacityTo, scrollYProgress }: StepPanelProps) {
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <div className={cn("flex h-14 w-14 items-center justify-center rounded-2xl mb-5", step.iconBg)}>
      ...
      {/* Tag pill */}
      <div className="mt-6 inline-flex w-fit items-center gap-2 rounded-full border px-4 py-1.5"
        style={{ borderColor: step.accent + "40", backgroundColor: step.accent + "10" }}>
        <span className="h-1.5 w-1.5 rounded-full animate-pulse" style={{ backgroundColor: step.accent }} />
...
export function ScrollShowcase() {
  ...
  // Sync scroll progress to ref — no re-renders from scroll
  useMotionValueEvent(scrollYProgress, "change", (v) => {
    progressRef.current = v;
    const scene = v < 0.33 ? 0 : v < 0.66 ? 1 : 2;
    // Only update activeScene (React state) when scene actually changes
    // This is safe: changes at most 2 times total during scroll, not per frame
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "w-full",
          isMobile ? "relative py-16" : "sticky top-0 h-screen overflow-hidden"
        )}
      >
        {/* Background */}
          ...
          {isMobile ? (
              ...
              <div className="space-y-16">
                {STEPS.map((step) => (
                  <div key={step.num} className="flex gap-5">
                    <div className={cn("flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl mt-1", step.iconBg)}>
                      <step.Icon className={cn("h-5 w-5", step.color)} />
                    </div>
                    <div>
                      <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Step {step.num}</span>
                      <h3 className="mt-1 text-xl font-bold text-foreground">{step.title}</h3>
                      <p className="mt-2 text-muted-foreground leading-relaxed text-sm">{step.body}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 gap-8 h-full items-center">
              ...
              <div className="relative h-full flex items-center">
                ...
                {/* All 3 steps layered, opacity controlled by scroll */}
                <div className="relative w-full h-64">
                  <StepPanel
                    step={STEPS[0]}
                    opacityFrom={0} opacityPeakStart={0.06} opacityPeakEnd={0.27} opacityTo={0.36}
                    scrollYProgress={scrollYProgress}
                  />
                  <StepPanel
                    step={STEPS[1]}
                    opacityFrom={0.32} opacityPeakStart={0.42} opacityPeakEnd={0.6} opacityTo={0.69}
                    scrollYProgress={scrollYProgress}
                  />
                  <StepPanel
                    step={STEPS[2]}
                    opacityFrom={0.65} opacityPeakStart={0.75} opacityPeakEnd={0.95} opacityTo={1.0}
                    scrollYProgress={scrollYProgress}
                  />
                </div>
              </div>
              ...
              <div className="relative h-[90vh]">
                {/* Dark backing — prevents white flash while WebGL context initialises */}
                <div className="absolute inset-0 rounded-3xl bg-zinc-950/60" />
                ...
                />
                {/* absolute inset-0 gives the Canvas explicit pixel bounds to fill */}
                <div className="absolute inset-0">
